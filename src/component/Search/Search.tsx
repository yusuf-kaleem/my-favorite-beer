...
function Search(){
        ...
        return {
          ...
          label: (
            ...
            >
              ...
              <span><img height={30} width={20} src={_.image_url}></img></span>
            </div>
          ),
        };
      });
  ...
  const onSelect = (value: string) => {
    axios.get(`https://api.punkapi.com/v2/beers?beer_name=${value}`).then((data) => {
      setBeer(data.data[0])
      setShowModal(true)
    })
      ...
      <div style={{ display: 'flex', alignItems: 'center',marginLeft:'auto' }}>
        ...
        <div style={{ display: 'flex' ,color:"#333" }}>
          ...
          <div style={{ display: 'flex', flex: 1 }}>
            ...
            <Form onSubmit={(v: any) => { submit(v) }} style={{
              width: "200px",
              height: "30px",
              fontSize: "12px"
            }}>
              <h2 style={{ fontSize: "20px", fontWeight: 'bold' }}>{beer.name}</h2>
              ...
              <button style={{ background: "#4893f0", color: "#fff", border: "none", fontSize: 14, padding: "3px 10px", borderRadius: 3 }}> Save </button>
            </Form>
          </div>
        </div>
      </Modal>
